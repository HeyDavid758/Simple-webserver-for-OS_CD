...
{ 
    ...
    fprintf(fp,"Gettimeofday: Getting the apply imformation  Time:%.8f ms\n",time_use);
   ...
   /*  根据预定义在 extensions 中的文件类型，检查请求的文件类型是否本服务器支持  */ 
    ...
    gettimeofday(&end,NULL);
    time_use=(double)(end.tv_usec-start.tv_usec)/1000;
    fprintf(fp,"Gettimeofday:Generating HTTP response Time:%.8f ms\n",time_use);
   ...
   /*  不停地从文件里读取文件内容，并通过 socket 通道向客户端返回文件内容*/ 
    ...
    gettimeofday(&end,NULL);
    time_use=(double)(end.tv_usec-start.tv_usec)/1000;
    fprintf(fp,"Gettimeofday: Return imformation to Client Time:%.8f ms\n\n",time_use);
    fclose(fp);
